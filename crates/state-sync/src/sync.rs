...
pub trait StateSync: Send + Sync {
    ...
    /// Drain pending deltas that were generated by broadcast_changes().
    /// Returns a list of (peer_id, delta) pairs that need to be sent over the transport.
    fn drain_pending_deltas(&mut self) -> Vec<(AgentId, Delta)>;
}
...
pub struct DefaultStateSync {
    ...
    // Pending deltas generated by broadcast_changes(), awaiting transport dispatch.
    pending_deltas: Vec<(AgentId, Delta)>,
}
...
impl StateSync for DefaultStateSync {
    ...
    async fn broadcast_changes(&mut self) -> Result<()> {
        // Generate deltas for all known peers and store them in a pending queue.
        // The caller (e.g., IntegrationAdapter) is responsible for actually sending
        // these deltas over the transport.
        let peers: Vec<AgentId> = self.peer_clocks.keys().cloned().collect();
        for peer in peers {
            let known_vclock = self.peer_clocks.get(&peer)
                .cloned()
                .unwrap_or_default();
            if let Some(delta) = self.map.delta_since(&known_vclock) {
                let delta = Delta::new(self.local_agent, delta.ops, delta.vclock);
                // Store in pending deltas for the caller to retrieve
                self.pending_deltas.push((peer, delta));
            }
        }
        tracing::debug!("Generated {} pending deltas for broadcast", self.pending_deltas.len());
        Ok(())
    }
...
mod tests {
    ...
    async fn test_default_state_sync_new() {
        ...
        assert!(sync.pending_deltas.is_empty());
    }
    ...
    #[tokio::test]
    async fn test_broadcast_changes_generates_deltas() {
        let mut sync = DefaultStateSync::new(AgentId(1));
        // Add some data
        sync.map.set("key1", json!("value1"), AgentId(1));
        // Register a peer with a known clock
        sync.update_peer_clock(AgentId(2), &VectorClock::default());
        let result = sync.broadcast_changes().await;
        assert!(result.is_ok());
        // Should have generated a pending delta for peer 2
        let pending = sync.drain_pending_deltas();
        assert_eq!(pending.len(), 1);
        assert_eq!(pending[0].0, AgentId(2));
    }
    ...
    #[tokio::test]
    async fn test_drain_pending_deltas() {
        let mut sync = DefaultStateSync::new(AgentId(1));
        sync.map.set("key1", json!("value1"), AgentId(1));
        sync.update_peer_clock(AgentId(2), &VectorClock::default());
        sync.broadcast_changes().await.unwrap();
        let pending = sync.drain_pending_deltas();
        assert_eq!(pending.len(), 1);
        // Second drain should be empty
        let empty = sync.drain_pending_deltas();
